...
from config import BINANCE_FUTURES_URL, BINANCE_API_KEY, BINANCE_SECRET_KEY, DEFAULT_SYMBOL, symbol_to_pair
...
def fetch_market_context(symbol: str = DEFAULT_SYMBOL) -> dict:
    ...
    base_asset = (symbol_to_pair(symbol).split("/", 1)[0] or symbol)
    ctx: dict = {"symbol": symbol, "base_asset": base_asset}
...
def format_market_context(ctx: dict) -> str:
    ...
    base_asset = ctx.get("base_asset") or "BTC"
    ...
    if taker_hist:
        ...
        lines.append(
            ...
            f"(최근: 매수 {last['buy']:,.1f} {base_asset} / 매도 {last['sell']:,.1f} {base_asset})"
        )
    ...
    if ctx.get("cvd_current") is not None:
        ...
        lines.append(
            f"  CVD 24h 누적 델타: {cvd_24h:+,.1f} {base_asset} ({cvd_dir}) / "
            f"최근 4h 델타: {cvd_4h:+,.1f} {base_asset} ({cvd4_dir})"
        )
        lines.append(
    ...
    if ctx.get("ob_imbalance") is not None:
        ...
        lines.append(
            ...
            f"매수잔량 {bid_q:,.1f} {base_asset} / 매도잔량 {ask_q:,.1f} {base_asset}"
            f"  ※ 스냅샷 — 대형 호가 취소 시 즉시 변동, 단기 방향 편향 참고"
        )

    # ── 오픈 인터레스트 ──────────────────────────
    if ctx.get("open_interest") is not None:
        lines.append(f"  오픈 인터레스트 Binance: {ctx['open_interest']:,.1f} {base_asset}")
        if ctx.get("bybit_oi") is not None:
            lines.append(f"  오픈 인터레스트 Bybit:   {ctx['bybit_oi']:,.1f} {base_asset}")
        if ctx.get("combined_oi") is not None:
            lines.append(
                f"  오픈 인터레스트 합산(Binance+Bybit): {ctx['combined_oi']:,.1f} {base_asset}"
