...
Binance 실시간 선물 데이터 + OpenAI 분석 + 거시경제 지표를 하나의 웹 인터페이스로 제공합니다.
...
### 🤖 OpenAI 분석
- 멀티 타임프레임 기술적 지표를 종합한 OpenAI 분석 리포트
- 롱/숏/관망 신호 + 신뢰도 점수 출력
- 진입가 · 목표가 · 손절가 자동 산출
- AI와 후속 질문 채팅 가능 (분석 컨텍스트 공유)
- 실시간 차트 기반 GPT 의견 요청 및 매매 시나리오 작성 패널
...
- OpenAI API 키 (필수) / Binance API (선택)
...
| 키 | 필수 여부 | 발급 주소 |
|----|----------|----------|
| OpenAI API Key | ✅ 필수 | [platform.openai.com/api-keys](https://platform.openai.com/api-keys) |
...
`.env` 파일이 없거나 OpenAI 키가 없으면 시작 시 설정 모달이 자동으로 뜹니다.
