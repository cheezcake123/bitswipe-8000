...
- the eight media files that originally entered the repo in snapshot commit `1540ab4dbda206535a477623c9f39310d2ba2007` had UNKNOWN provenance
- the current versions of those eight media paths have now been replaced with deterministic geometric assets generated by `scripts/generate_brand_assets.py`
- current asset hashes and generation provenance are recorded in `STATIC_ASSET_AUDIT.md`
- the former UNKNOWN media bytes remain in historical Git commits; this work does not rewrite repository history
...
The current media tree now has reproducible repository-local generation evidence, but replacement does not erase the prior UNKNOWN asset versions from public Git history.
...
2. reduce upstream implementation dependence through independently designed, tested rewrites where appropriate
...
6. decide the downstream project's own license separately from upstream attribution obligations and historical media provenance
