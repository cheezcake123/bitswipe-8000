...
def normalize_symbol(raw_symbol: str) -> str:
    ...
    if not any(symbol.endswith(q) for q in ("USDT", "USDC", "FDUSD", "BUSD", "TUSD")):
        symbol = f"{symbol}USDT"
    if not _SYMBOL_RE.fullmatch(symbol):
        raise HTTPException(status_code=400, detail="Invalid Binance futures symbol")
    return symbol


def active_symbol() -> str:
    return _market_stream.symbol if "_market_stream" in globals() else DEFAULT_SYMBOL


def fetch_binance_futures_symbols() -> list[str]:
    now = time.time()
    cached = _SYMBOL_UNIVERSE_CACHE.get("symbols") or []
    if cached and now - float(_SYMBOL_UNIVERSE_CACHE.get("ts") or 0.0) < _SYMBOL_UNIVERSE_TTL_SECS:
        return list(cached)

    resp = _http.get(f"{runtime_config.BINANCE_FUTURES_URL}/fapi/v1/exchangeInfo", timeout=10)
    ...
    symbols = sorted({
        ...
        and item.get("quoteAsset") in {"USDT", "USDC", "FDUSD", "BUSD", "TUSD"}
        and _SYMBOL_RE.fullmatch(str(item.get("symbol") or "").upper())
    })
    _SYMBOL_UNIVERSE_CACHE.update({"ts": now, "symbols": symbols})
    return symbols
...
def _market_overview(tf_data: dict, price: float, last_update: str | None, symbol: str | None = None) -> dict:
    symbol = normalize_symbol(symbol or active_symbol())
    h1 = tf_data.get("1h")
    if h1 is None:
    ...
    overview = {
        "symbol":      symbol,
        "pair_label":  symbol_to_pair(symbol),
        "price":       price,
...
def _build_account_payload(symbol: str | None = None) -> dict:
    requested_symbol = normalize_symbol(symbol) if symbol else None
    display_symbol = requested_symbol or active_symbol()
    ctx = fetch_account_context(requested_symbol)
    return {
        "updated_at": _now_label(),
        "symbol": display_symbol,
        "pair_label": symbol_to_pair(display_symbol),
        "active_symbol": active_symbol(),
        "account_scope": "symbol" if requested_symbol else "all",
        **ctx,
    }
...
def build_market_payload(
    ...
    if include_overview:
        payload = _market_overview(tf_data, price, last_update, symbol=symbol)
        payload["fibs"] = fibs
    else:
        payload = {
            "symbol":      symbol,
...
def _extract_leverage(value):
    ...
    try:
        ...
        if isinstance(value, str):
            ...
            cleaned = value.lower().replace("x", "").replace("배", "").strip()
...
def _build_risk_guard(analysis: dict) -> dict:
    ...
    try:
        ...
        entry = _safe_float(trade_levels.get("entry"))

        stop = _safe_float(trade_levels.get("stop"))

        target = _safe_float(trade_levels.get("target"))



        leverage = _extract_leverage(analysis.get("claude_leverage"))
        ...
        if not side:
            ...
            return {
                ...
                "errors": ["signal 값이 없어 risk_guard를 실행하지 않았습니다."],
        ...
        if entry is None or stop is None:
            ...
            return {
                ...
                "errors": ["진입가 또는 손절가가 없어 risk_guard를 실행하지 않았습니다."],
        ...
        return {
            ...
            "errors": [f"risk_guard 실행 중 오류: {exc}"],
...
def _build_payload(tf_data: dict, price: float, analysis: dict, symbol: str | None = None) -> dict:
    symbol = normalize_symbol(symbol or active_symbol())
    payload = build_market_payload(tf_data, price, include_overview=True, symbol=symbol)
    return {
        **payload,
        ...
        # Risk Guard: 손절가 방향 / 손익비 / 포지션 크기 기계 검증
        "risk_guard": _build_risk_guard(analysis),
        # 과거 유사 상황 (BM25 회상 결과)
...
class MarketStreamManager:
    ...
    async def set_symbol(self, symbol: str) -> bool:
        ...
        async with self._lock:
            ...
            self._needs_full_refresh = False
            self._pending_kline.clear()
            self._kline_running.clear()
            self._forming_kline.clear()
            self._trade_count = 0
            self._kline_count = 0
    ...
    async def _flush_price_update(self):
        ...
        await self._broadcast({"type": "price", "data": {
            "symbol": self.symbol,
            "pair_label": symbol_to_pair(self.symbol),
            "price": price,
            "last_update": last_update,
        }})
    ...
    async def get_status(self, include_latest: bool = False) -> dict:
        async with self._lock:
            ...
            if (
                ...
                latest = json.loads(self._latest_result_json)
                if (latest.get("symbol") or DEFAULT_SYMBOL) == active_symbol():
                    response["latest_result"] = latest
            return response

    async def get_latest_result(self, symbol: str | None = None) -> dict | None:
        async with self._lock:
            if self._latest_result_json is None:
                return None
            latest = json.loads(self._latest_result_json)
            if symbol and (latest.get("symbol") or DEFAULT_SYMBOL) != symbol:
                return None
            return latest
    ...
    async def _run_job(self, job_id: str):
        ...
        try:
            ...
            if _market_stream.is_ready():
                ...
                tf_data = await loop.run_in_executor(_executor, _fetch_all, symbol)
                try:
                    price = await loop.run_in_executor(_executor, fetch_current_price, symbol)
                except Exception:
                    price = None
            ...
            payload = await loop.run_in_executor(
                _executor, functools.partial(_build_payload, tf_data, price, analysis, symbol=symbol)
            )
            await self._complete(job_id, payload)
            from notifier.trade_alert import maybe_send_trade_alert
            await asyncio.to_thread(maybe_send_trade_alert, payload)
...
class SymbolSetRequest(BaseModel):
    symbol: str
...
async def symbols_get():
    try:
        symbols = await asyncio.to_thread(fetch_binance_futures_symbols)
        return {"symbols": symbols, "common_symbols": COMMON_SYMBOLS}
    except Exception as exc:
        return {
...
@app.post("/api/symbol")
async def symbol_set(body: SymbolSetRequest):
    symbol = normalize_symbol(body.symbol)
    try:
        await asyncio.to_thread(fetch_current_price, symbol)
    except Exception as exc:
        raise HTTPException(
            status_code=400,
            detail=f"{symbol} is not available from Binance USD-M Futures: {exc}",
        )

    await _analysis_manager.stop()
    await _analysis_manager.clear_current_job()
    changed = await _market_stream.set_symbol(symbol)

    try:
        await asyncio.wait_for(_market_stream.wait_until_ready(), timeout=35)
        snapshot = await _market_stream.get_full_snapshot()
    except Exception as exc:
...
async def debug_price():
    ...
    return {
        "symbol": _market_stream.symbol,
        "pair_label": symbol_to_pair(_market_stream.symbol),
        "price": price,
        "last_update": last_update,
...
class GptOpinionRequest(BaseModel):
    question: str
    timeframe: str | None = None


class GptScenarioRequest(BaseModel):
    timeframe: str | None = None


def _pct_change(series, bars: int) -> float | None:
    if series is None or len(series) <= bars:
        return None
    cur = _safe(series.iloc[-1])
    prev = _safe(series.iloc[-1 - bars])
    if cur is None or prev in (None, 0):
        return None
    return round((cur / prev - 1) * 100, 3)


def _chart_tf_digest(tf: str, df) -> dict:
    if df is None or df.empty:
        return {"timeframe": tf, "available": False}

    last = df.iloc[-1]
    close_v = _safe(last.get("close"))
    volume_v = _safe(last.get("volume"))
    volume_ma = _safe(last.get("volume_ma"))
    volume_ratio = None
    if volume_v is not None and volume_ma not in (None, 0):
        volume_ratio = round(volume_v / volume_ma, 3)
    ...
    return {
        ...
        "available": True,
        "last_close": close_v,
        "change_1_bar_pct": _pct_change(df["close"], 1) if "close" in df else None,
        "change_3_bars_pct": _pct_change(df["close"], 3) if "close" in df else None,
        "change_10_bars_pct": _pct_change(df["close"], 10) if "close" in df else None,
        ...
        "atr_pct": round((_safe(last.get("atr")) or 0) / close_v * 100, 3) if close_v else None,
        "volume_ratio": volume_ratio,
        "last_8_candles": last_candles,
    }


def _analysis_digest(latest: dict | None) -> dict:
    if not latest:
        return {}
    raw_text = str(latest.get("raw_text") or "")
    return {
        "analysis_time": latest.get("analysis_time"),
        "signal": latest.get("signal"),
        "confidence": latest.get("confidence"),
        "trade_levels": latest.get("trade_levels") or {},
        "trading_signal": latest.get("trading_signal") or {},
        "report_excerpt": raw_text[:3500],
    }


async def _build_gpt_context(timeframe: str | None = None) -> dict:
    loop = asyncio.get_event_loop()
    symbol = active_symbol()
    if _market_stream.is_ready():
        tf_data, price = await _market_stream.get_analysis_inputs()
    ...
    requested_tf = timeframe if timeframe in TIMEFRAMES else None
    latest = await _analysis_manager.get_latest_result(symbol=symbol)
    ...
    return {
        ...
        "timeframes": {tf: _chart_tf_digest(tf, tf_data.get(tf)) for tf in TIMEFRAMES},
        "fibonacci": fibs,
        "latest_analysis": _analysis_digest(latest),
    }


def _call_openai_text(system_prompt: str, user_prompt: str, max_tokens: int = 1600) -> str:
...
@app.post("/api/gpt/opinion")
async def gpt_opinion(body: GptOpinionRequest):
    if not runtime_config.OPENAI_API_KEY:
        raise HTTPException(status_code=400, detail="OPENAI_API_KEY가 설정되어 있지 않습니다.")
    ...
    if len(question) > 1200:
        raise HTTPException(status_code=400, detail="질문은 1200자 이하로 입력하세요.")

    context = await _build_gpt_context(body.timeframe)
    pair_label = context.get("pair_label") or symbol_to_pair(active_symbol())
    system_prompt = (
        f"너는 {pair_label} 선물 단기 트레이딩 보조 애널리스트다. "
        "사용자의 질문에 현재 차트 데이터와 최신 분석 결과만 근거로 답한다. "
        "불확실하면 불확실하다고 말하고, 진입/청산/무효화 기준을 숫자로 제시한다. "
        "수익이나 승률을 보장하지 말고 한국어로 간결하게 답한다."
    )
    user_prompt = (
        f"[사용자 질문]\n{question}\n\n"
        f"[실시간 차트 컨텍스트]\n{json.dumps(context, ensure_ascii=False)}"
    )
    answer = await asyncio.to_thread(_call_openai_text, system_prompt, user_prompt, 1400)
    return {"ok": True, "answer": answer, "timeframe": context.get("requested_timeframe")}


@app.post("/api/gpt/scenario")
async def gpt_scenario(body: GptScenarioRequest):
    if not runtime_config.OPENAI_API_KEY:
        raise HTTPException(status_code=400, detail="OPENAI_API_KEY가 설정되어 있지 않습니다.")
    context = await _build_gpt_context(body.timeframe)
    tf = context.get("requested_timeframe") or "current"
    pair_label = context.get("pair_label") or symbol_to_pair(active_symbol())
    system_prompt = (
        f"너는 {pair_label} 선물 실시간 차트 기반 시나리오 작성자다. "
        "현재 데이터에서 기대값과 승률 우위가 가장 높아 보이는 단 하나의 시나리오를 고른다. "
        "우위가 불충분하면 억지로 매매하지 말고 관망 시나리오를 낸다. "
        "반드시 한국어로, 실행 가능한 가격 조건 중심으로 작성한다. "
        "수익이나 승률을 보장하지 않는다."
    )
    user_prompt = (
        f"[요청 타임프레임]\n{tf}\n\n"
    ...
    scenario = await asyncio.to_thread(_call_openai_text, system_prompt, user_prompt, 1700)
    return {"ok": True, "scenario": scenario, "timeframe": context.get("requested_timeframe")}
...
async def reflect_endpoint():
    ...
    try:
        price_now = await asyncio.to_thread(fetch_current_price, active_symbol())
    except Exception as exc:
...
# ─── 시장 심리 스냅샷 캐시 (5분 TTL) ─────────────────────────
_sentiment_cache: dict = {"data": None, "ts": 0.0, "symbol": None}
...
async def market_sentiment_endpoint():
    ...
    symbol = active_symbol()
    if (
        _sentiment_cache["data"] is None
        or _sentiment_cache.get("symbol") != symbol
        or now - _sentiment_cache["ts"] > _SENTIMENT_TTL
    ):
        from market_context import fetch_market_context
        ctx = await asyncio.to_thread(fetch_market_context, symbol)
        _sentiment_cache["data"] = ctx
        _sentiment_cache["ts"]   = now
        _sentiment_cache["symbol"] = symbol
...
async def setup_status():
    ...
    return {
        "ai":       bool(_cfg.OPENAI_API_KEY or _cfg.CLAUDE_API_KEY),
        "openai":   bool(_cfg.OPENAI_API_KEY),
...
async def setup_save(body: SetupSaveRequest, request: Request):
    ...
    updates = {
        "OPENAI_API_KEY":     runtime_config.sanitize_env_value(body.openai_api_key),
