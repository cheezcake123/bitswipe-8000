...
def verify_owner_session(token: object, runtime_config) -> bool:
    ...
    try:
        ...
        return False
    now = int(time.time())
    if version != _TOKEN_VERSION or expires_at < now:
        return False
    if expires_at > now + MAX_SESSION_TTL_SECONDS + 60:
        return False
...
def _safe_next(raw: object) -> str:
    ...
    return value if value == PRIVATE_ACCOUNT_PATH else PRIVATE_ACCOUNT_PATH
...
def arm_fastapi_owner_security(runtime_config) -> None:
    """Install owner security on FastAPI apps created after this call, once per app."""
