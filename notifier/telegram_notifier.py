...
try:
    ...
    def register_successful_trade_alert(prepared, response, *, env=None):
...
def send_telegram_message(
    text: str,
    *,
    registration_plan: Optional[Mapping[str, Any]] = None,
) -> Dict[str, Any]:
    ...
    if not token:
        return {"ok": False, "error": "TELEGRAM_BOT_TOKEN이 설정되어 있지 않습니다."}
    if not chat_id:
        return {"ok": False, "error": "TELEGRAM_CHAT_ID가 설정되어 있지 않습니다."}
