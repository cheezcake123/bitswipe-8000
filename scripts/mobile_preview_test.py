...
ALLOWED_DIFFS = {
    "http_client.py", "owner_auth.py", "scripts/mobile_preview_test.py", "scripts/public_beta_security_test.py",
...
class FrontendPreviewSafetyTest(unittest.TestCase):
    ...
    def test_01_exact_version_boundaries_and_unchanged_server(self) -> None:
        ...
        security_files = {
            "http_client.py", "owner_auth.py", "scripts/mobile_preview_test.py",
            "scripts/public_beta_security_test.py", "static/assets/decision-account-preview.html",
        }
        self.assertEqual(changed_since(SECURITY_BASE), security_files)
        self.assertEqual(changed_since(PERSONAL_CONTEXT_BASE), security_files | {
            "static/assets/decision-account-preview.css", "static/assets/decision-account-preview.js",
        })
        self.assertEqual(changed_since(PUBLIC_SHELL_BASE), security_files | {
            "static/assets/mobile-preview-foundation.js", "static/assets/decision-account-preview.css",
            "static/assets/decision-account-preview.js",
        })
        self.assertEqual(changed_since(JOURNAL_BASE), security_files | {
        ...
        for path in ("server.py", "config.py", "static/index.html", "static/assets/bitswipe.css"):
            self.assertEqual(
    ...
    def test_02_root_mobile_and_static_mount_contract(self) -> None:
        ...
        self.assertIn("/mobile-preview", routes)
    ...
    def test_03_mobile_read_only_demo_privacy_and_stream_contract(self) -> None:
        ...
        branch = body.index("if (demoMode)")
        ret = body.index("return;", branch)
        self.assertLess(ret, body.index("connectMarketStream()"))
        self.assertLess(ret, body.index("loadLiveReads()"))
    ...
    def test_04_mobile_decision_semantics_and_accessibility(self) -> None:
        ...
        compact = re.sub(r"\s+", " ", grade.group("body"))
    ...
    def test_05_macro_foundation_and_dynamic_rendering_contracts(self) -> None:
    ...
    def test_06_public_decision_network_navigation_and_layout(self) -> None:
    ...
    def test_07_journal_public_read_only_truthfulness_and_layout(self) -> None:
        ...
        self.assertNotIn("EventSource", self.journal_js)
        self.assertNotIn("/api/account", self.journal_js)
    ...
    def test_08_private_account_scope_and_public_separation(self) -> None:
    ...
    def test_09_private_account_network_demo_and_storage_boundary(self) -> None:
        ...
        branch = body.index("if (demoMode)")
        ret = body.index("return;", branch)
        self.assertLess(ret, body.index("connectAccountStream()"))
        self.assertLess(ret, body.index("loadAnalysisReads()"))
    ...
    def test_10_private_account_scenario_link_and_auth_copy(self) -> None:
    ...
    def test_11_private_account_responsive_accessibility(self) -> None:
    ...
    def test_12_tradfi_remains_context_not_candidate(self) -> None:
    ...
    def test_13_scenario_grade_is_not_win_rate(self) -> None:
    ...
    def test_14_mobile_privacy_preference_does_not_persist_account_snapshot(self) -> None:
        writes = re.findall(r"localStorage\.setItem\(([^,\n]+),\s*([^)]+)\)", self.mobile_js)
        self.assertTrue(writes)
        for key_expression, value_expression in writes:
            self.assertNotRegex(key_expression + " " + value_expression, r"state\.account|DEMO_ACCOUNT|wallet|balance|positions|notional|pnl|leverage")
