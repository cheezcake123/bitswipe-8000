...
except ImportError:  # Claude is optional when OpenAI is configured.
    anthropic = None  # type: ignore
    AnthropicAPIStatusError = _MissingSDKStatusError  # type: ignore
try:
    from openai import OpenAI
    from openai import APIStatusError as OpenAIAPIStatusError
except ImportError:  # OpenAI is optional when Claude fallback is configured.
...
def _pair_label_for(symbol: Optional[str] = None) -> str:
    return symbol_to_pair((symbol or DEFAULT_SYMBOL).upper())
...
def _build_context_blob(
    ...
    try:
        market_ctx = fetch_market_context(symbol or DEFAULT_SYMBOL)
    ...
    try:
        account_ctx = fetch_account_context(symbol or DEFAULT_SYMBOL)
...
def build_prompt(
    ...
    """
    pair_label = _pair_label_for(symbol)
    context_blob = _build_context_blob(multi_tf_data, macro_snapshot, symbol=symbol)
...
def _system_prompt_for(pair_label: str) -> str:
    if pair_label == PAIR_LABEL:
        return SYSTEM_PROMPT
    return SYSTEM_PROMPT.replace(PAIR_LABEL, pair_label)


def _system_prompt_text(system_value: Any) -> str:
    ...
    return str(system_value or "")


def _system_prompt_param(pair_label: str | None = None):
    """Anthropic prompt caching: 정적 system prompt를 cache breakpoint로 표시."""
    prompt = _system_prompt_for(pair_label or PAIR_LABEL)
...
def _configured_ai_provider() -> str:
    ...
    if runtime_config.CLAUDE_API_KEY:
        return "claude"
    raise RuntimeError("OPENAI_API_KEY 또는 CLAUDE_API_KEY를 설정하세요.")


def _parse_openai_tool_json(message: Any) -> Optional[dict]:
    tool_calls = getattr(message, "tool_calls", None) or []
    for call in tool_calls:
        function = getattr(call, "function", None)
        if getattr(function, "name", None) != "record_analysis":
            continue
...
class _OpenAIChatAdapter:
    """Expose OpenAI Chat Completions through the small Anthropic-like surface used below."""
    ...
    def create(self, **kwargs: Any) -> Any:
        system_prompt = _system_prompt_text(kwargs.get("system") or SYSTEM_PROMPT)
        ...
        if kwargs.get("tools"):
            openai_kwargs["tools"] = [_openai_tool_schema()]

        completion = self._client.chat.completions.create(**openai_kwargs)
        ...
        if raw_text:
            blocks.append(SimpleNamespace(type="text", text=raw_text))

        tool_json = _parse_openai_tool_json(message)
        if tool_json is not None:
            blocks.append(SimpleNamespace(type="tool_use", name="record_analysis", input=tool_json))
...
def analyze_with_claude(
    ...
    plain_system_prompt = _system_prompt_for(pair_label)
    request_kwargs = {
        "model": runtime_config.CLAUDE_MODEL,
        "max_tokens": _analyst_max_tokens,
        "system": _system_prompt_param(pair_label),
    ...
    for attempt in range(max_retries):
        try:
            ...
            if e.status_code == 400 and request_kwargs.get("system") != plain_system_prompt:
                # 모델/SDK 조합이 cache_control system block을 받지 못하면
                # 동일 프롬프트를 일반 system 문자열로 한 번 더 시도한다.
                request_kwargs["system"] = plain_system_prompt
                continue
            if e.status_code in (429, 500, 502, 503, 504, 529) and attempt < max_retries - 1:
                time.sleep(wait)
                wait *= 2  # 10 → 20 → 40 → 80초
            else:
                raise
...
def run_full_analysis(
    ...
    context_blob, raw_ctx = _build_context_blob(
        multi_tf_data, macro_snapshot, return_raw=True, symbol=symbol
    )
